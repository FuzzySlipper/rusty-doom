...
### Recipe sprite animation and viewmodel layout

`LoadingBayRecipeAnimation` adapts the 35 Hz pistol/shotgun and POSS/TROO state timings from the offline reading reference `doom.ts` revision `0d88ba912f7b084a05b776a19801d45f383cef20` (`src/doom/doom/info/states.ts`, `src/doom/play/p-sprite.ts`, `src/doom/rendering/things.ts`). No donor runtime or source file is shipped. Weapon effects occur at the source discharge frame; independent fullbright muzzle-flash patches share the gun origin. World sprites retain patch pivots; origins below the patch become a vertical placement offset within Engine's normalized-pivot contract.

`scripts/authoring/generate-recipe-sprites.py` copies the retained RGBA pixels from `content/doom-e1m1/sprites/weapons.png` onto sixteen transparent 320×168 canvases, each surrounded by a two-pixel transparent gutter using the manifest's original patch offsets. It writes `content/loading-bay/recipe-viewmodels.png` and typed frame metadata. It introduces no painted/resampled source pixels. Engine applies nearest sampling and the classic 1.2 vertical pixel aspect, fits the common canvas, and anchors it at the screen bottom beneath the compact status bar. This adapts the original screen layout to the current HUD rather than claiming exact legacy view-window/bob/raise/lower behavior. `pnpm run check:provenance` deterministically regenerates and compares both outputs.
Generated atlas: 324×2752, 58808 bytes, SHA-256 `55dcb11d07dc0b5fe69eba3a297b76b18132cab07a981f19ba8cb0fb439e9016`. Frame UVs use interior texel centers to prevent neighboring-frame bleed.
