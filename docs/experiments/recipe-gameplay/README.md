...
Controls: WASD/mouse, click/RT fire, E/controller X use doors and exit, 1 fist, 2 pistol, 3 shotgun after collecting one, R restart. Right-stick, RT, or X selects bounded gamepad target assistance: a neutral stick retains focus without camera tracking, while pointer or keyboard look clears that mode. RT still resolves its final direction through the normal collision ray. F3/backquote opens the collapsed debug block. Its console and renderer metrics use the packaged Engine widgets. `loading-bay.diagnostics true` enables HUD diagnostic samples at 0.25-second intervals; `false` disables them. Opening Debug alone does not enable that stream. `loading-bay.readout` remains a one-shot read and includes HUD publication and trigger reconciliation counts. `interaction.help`, `interaction.inspect`, and `interaction.use <id> <revision>` expose the same fresh-validated door/exit action without pixel hunting.
