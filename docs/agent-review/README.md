...
`rusty-doom/agent-review-workflow` owns the policy; the files here are the
...
need. Content-admission and provenance gravity (typed catalogs, project hashes,
semantic checks, schema versions) leaks into trusted single-player runtime paths
as delegate lists, Capture reads, defensive copies, mutate-validate-rollback,
revision guards, snapshots, and save-shaped reads. Campaign #8376 removes that
machinery; this lane holds the removal until trusted-path gravity is established
(see task #8376 and, once landed, `docs/gameplay-design.md`).
...
| Lane | Use when |
| --- | --- |
| [Ownership and values](lanes/ownership-and-values.md) | the change crosses a C#/Engine/TS seam, adds tuning, or touches E1M1 definitions or provenance |
